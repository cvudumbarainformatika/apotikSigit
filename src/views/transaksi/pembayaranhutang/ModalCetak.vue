      ...
      <div id="printArea" ref="printArea" class=" max-w-full bg-white text-black mx-auto  p-4 print-a4">
        ...
        <div class="flex items-start justify-between gap-6">
          ...
          <div class="text-right">
            ...
            <div class="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
              ...
              <div class="font-medium">{{ data?.nopelunasan || '-' }}</div>
              <div class="text-gray-500">Tanggal</div>
              <div class="font-medium">{{ formatDateIndo(data?.tgl_pelunasan) }}</div>
            </div>
          </div>
        </div>
        ...
        <!-- <div class="grid sm:grid-cols-2 gap-6 text-sm">
          <div class="p-4 rounded-xl bg-gray-50">
            ...
            <div class="mt-1 font-medium">{{ data || '-' }}</div>
        ...
        <div class="mt-6">
          <table class="w-full text-sm border-separate [border-spacing:0]">
            <thead>
              <tr class="text-left">
                <th class="th">#</th>
                <th class="th">No Penerimaan</th>
                <th class="th text-right">Penerimaan (Rp.)</th>
            ...
            <tbody>
              <tr v-for="(it, idx) in data?.rinci" :key="idx" class="align-top">
                ...
                <td class="td">
                  <div class="font-medium">{{ it?.nopenerimaan || 0 }}</div>
                  <div class="text-gray-500">No Faktur {{ it.nofaktur }}</div>
                </td>
                <td class="td text-right">{{ formatRupiah(it?.nominal || 0) }}</td>
                <td class="td text-right">{{ formatRupiah(it?.pajak) || '-' }}</td>
                <td class="td text-right">{{ it?.diskon ? formatRupiah(it?.diskon) : '-' }}</td>
                <td class="td text-right">{{ formatRupiah(it?.total) || '-' }}</td>
              </tr>
              <tr v-if="data?.rincian?.length === 0">
                <td class="td text-center text-gray-500" colspan="8">Belum ada item pembayaran.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const totalSubtotal = computed(() => {
  const items = data?.value?.rinci || []
  return items.reduce((a, b) => a + Number(b?.nominal), 0)
})
const totalPajak = computed(() => {
  const items = data?.value?.rinci || []
  return items.reduce((a, b) => a + Number(b?.pajak), 0)
})
...
const totals = computed(() => {
  return (totalSubtotal.value + totalPajak.value - totalDiskon.value) || 0
})
