            ...
            <u-row flex1>
              <u-col gap="gap-0" class="w-full">
                <u-row flex1>
                  ...
                  <u-text class="font-bold" color="text-gray-500">{{ item?.notransaksi }}</u-text>
                  <!-- <u-text class="" color="text-primary">{{ item?.nomor_order }}</u-text> -->
                </u-row>
                <u-row>
                  <u-icon size="14" name="shopping-cart"></u-icon>
                  <u-text class="" color="text-gray-500">Rp. {{ formatRupiah(item?.nominal_setoran) }}</u-text>
                </u-row>
                <!-- <u-row class="mt-1">
...
const handleEdit = (item) => {
  props.store.form.nosetoran = item?.notransaksi
  props.store.form.total_setoran = item?.nominal_setoran
  props.store.form.rincian = item?.rinci
